...
(function () {
  ...
  function buildFinder() {
    ...
    // Collapse button — pointerdown stops drag detection; click does the toggle
    tog.addEventListener('pointerdown', (e) => e.stopPropagation());
    tog.addEventListener('click',       (e) => { e.stopPropagation(); togglePanel(); });

    // Header tap also collapses (but not via the buttons)
    panel.querySelector('h3').addEventListener('click', (e) => {
      if (e.target.closest('button')) return; // let buttons handle themselves
      togglePanel();
    });

    // Gear — same pattern: stop pointerdown so h3 doesn't also fire, then act on click
    const gearBtn = document.getElementById('ttt-gear');
    gearBtn.addEventListener('pointerdown', (e) => e.stopPropagation());
    gearBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      // Expand first if collapsed so the settings div becomes visible
